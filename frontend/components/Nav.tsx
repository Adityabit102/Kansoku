    ...
    <header className="sticky top-0 z-40 border-b border-line bg-canvas/80 backdrop-blur-md">
      <div className="mx-auto flex max-w-[1400px] items-center gap-8 px-6 md:px-10">
        <Link href="/" className="group flex shrink-0 items-center gap-2.5 py-4">
          <span className="text-lg leading-none text-accent transition-opacity duration-200 group-hover:opacity-80">
            観
          </span>
          ...
          {LINKS.map(({ href, label }) => {
            ...
            return (
              <Link
                ...
                className="relative whitespace-nowrap px-3 py-4 text-[13px] text-muted transition-colors duration-200 hover:text-ink data-[active=true]:text-ink"
                data-active={active}
              >
                {label}
                {active && (
